...
── VENDORED（原样复制，勿改）──────────────────────────────────────────
本文件逐字复制自官方 A-Mem robust 评测版（AgenticMemory 仓库的
`llm_text_parsers.py`），供 harness 的 MEMORY_MODE="official_eval" 档做**忠实复刻**：
prompts 与 parsers 与官方逐字一致，保证进化/建链算法与官方等价，可直接对标。
需要偏离官方（小智增量）时请改 memory.py 里的 lite/hybrid/persist 路径，不要动本文件。
"""
